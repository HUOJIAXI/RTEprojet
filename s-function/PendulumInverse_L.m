function  [sys, x0]  = PendulumInverse_L(t,x,u,flag,Mcart,m,l,g,J)
% * 6 continuous states
% * 1 inputs: u
...
if abs(flag) == 1 % If flag = 1, return state derivatives, xDot
    ...
    sys(2,1)= (u+m*l*x(2)*x(2)*(x(1))-(Mcart+m)*g*(x(1)))/(m*l*1-(Mcart+m)*(J+m*l*l)/(m*l*1));
    sys(3,1) = x(4); % x3=x x=\dot x
    sys(4,1) = g*(x(1))-(J+m*l*l)/(m*l*1)*(u+m*l*x(2)*x(2)*x(1)-(Mcart+m)*g*x(1))/(m*l*1-(Mcart+m)*(J+m*l*l)/(m*l*1));
