function  [sys, x0]  = PendulumInverse_estime(t,x,u,flag,Mcart,m,l,g,J,theta)
% * 6 continuous states
% * 1 inputs: u
...
if abs(flag) == 1 % If flag = 1, return state derivatives, xDot
    ...
    sys(1,1)= x(2)+l1*e; % x1=theta x2=\dot theta
    sys(2,1)= (v+m*l*x(2)*x(2)*sin(x(1))-(Mcart+m)*g*tan(x(1)))/(m*l*cos(x(1))-(Mcart+m)*(J+m*l*l)/(m*l*cos(x(1))))+l2*e;
    sys(3,1) = x(4)+l3*e; % x3=x x=\dot x
    sys(4,1) = g*tan(x(1))-(J+m*l*l)/(m*l*cos(x(1)))*(v+m*l*x(2)*x(2)*sin(x(1))-(Mcart+m)*g*tan(x(1)))/(m*l*cos(x(1))-(Mcart+m)*(J+m*l*l)/(m*l*cos(x(1))))+l4*e;
